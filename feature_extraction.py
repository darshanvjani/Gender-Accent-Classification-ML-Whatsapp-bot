...
def get_features(frequencies,y,sr):
  ...
  iqr      = q75 - q25
  y , sr = librosa.load(os.path.join(sample_wav_folder, wav_file))
  ...
  return nobs, mean, skew, kurtosis, median, mode, std, low, peak, q25, q75, iqr,rms,chroma_stft,spec_cent,spec_bw,rolloff,zcr,mfcc
...
def get_frequencies(myData,sample_wav_folder,date,user_name,gender,age_range,pronuncition):
  ...
  for wav_file in os.listdir(sample_wav_folder):
    ...
    for i in range(0,len(data),step):
      ...
      nobs, mean, skew, kurtosis, median, mode, std, low, peak, q25, q75, iqr,rms,chroma_stft,spec_cent,spec_bw,rolloff,zcr,mfcc = get_features(filtered_frequencies,y,sr)
      ...
      sample_dict = {'nobs':nobs, 'mean':mean, 'skew':skew, 'kurtosis':kurtosis,
                       ...
                       'rms':rms,'chroma_stft':chroma_stft,'spec_cent':spec_cent,
                       'spec_bw':spec_bw,'rolloff':rolloff,'zcr':zcr,'mfcc':mfcc,
      ...
      myData = myData.append(sample_dict,ignore_index=True)
...
def main():
  ...
  for i in range(n_samples):
    ...
    if os.path.isdir(sample_wav_folder): #some of the samples don't contain a wav folder (Ex: 'LunaTick-20080329-vf1')
      myData = get_frequencies(myData,sample_wav_folder,date,user_name,gender,age_range,pronuncition)
        ...
        #myData.loc[i] = pd.Series(feature_dict)

  myData.to_csv('myData_filtered_2.csv')
